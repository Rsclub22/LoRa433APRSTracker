...
# convert to upper where MeshCom/APRS expect uppercase callsigns; keep MeshCom
# handle/name case-sensitive for APRS-IS rendering
config.callsign = config.callsign.upper()
if isinstance(config.node_id, str):
    ...
    if RADIO_MODE in ("MESH", "DUAL"):
        ...
        radio.tx(
            build_mesh_position_frame(
                ...
                config.callsign,
                config.symbol,
                handle=config.mesh_handle,
                node_name=config.mesh_name,
                status=(config.mesh_status or None),
            )
        )
