...
def _conv_feet(alt_m):
    """Mirror conv_fuss from loop_functions.cpp (round up 0.1ft)."""

    fuss = alt_m * 10.0
    fuss = fuss * 3.28084
    return int((fuss + 5.0) / 10.0)


def _build_payload(fix, symbol, handle, node_name=None, status=None):
    """Render the APRS-style position payload used by MeshCom.

    Mirrors PositionToAPRS (loop_functions.cpp) for the "mesh" path that the
    APRS gateway consumes. Only a subset is implemented: location plus optional
    handle/status and altitude.
    """
    ...
    catxt = handle or ""
    cname = ("#" + node_name) if node_name else ""

    status_text = ""
    if status:
        status_text = status if status.startswith("/") else "/" + status

    altitude_text = ""
    if altitude is not None:
        alt_feet = max(0, _conv_feet(altitude))
        altitude_text = "/A={:06d}".format(alt_feet)
...
def _next_sequence():
    """Return a rolling sequence like node_msgid in MeshCom (0..999)."""

    global _mesh_sequence
    value = _mesh_sequence % 1000
    _mesh_sequence = (value + 1) % 1000
    return value
...
def _append_payload(buffer, payload_text):
    ...
    buffer.extend(payload_text.encode("utf-8"))
...
def build_mesh_position_frame(
    fix, node_id, callsign=None, symbol="/>", handle=None, node_name=None, status=None
):
    ...
    payload_text = _build_payload(fix, symbol, handle, node_name=node_name, status=status)
